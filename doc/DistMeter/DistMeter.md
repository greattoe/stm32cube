...
### HC-SR04 초음파 센서를 이용한 거리측정기

**STM32CubeMX**와 **STM32CubeIDE**를 이용하여  **NUCLEO-F103RB**보드에  HC-SR04초음파 센서와 I2C CLCD를 연결하고, NUCLEO 보드의 온보드 스위치(blue push button)를 누를 때마다 측정된 거리를 I2C CLCD에 표시하는 초음파 거리측정기를 구현해 보자.
...
I2C CLCD와 NUCLEO-F103RB 타겟보드와의 결선은 다음과 같이 연결한다.

| I2C CLCD | NUCLEO-F103RB |
| -------- | ------------- |
| SCL      | PB8           |
| SDA      | PB9           |
| GND      | GND           |
...
/* USER CODE BEGIN 2 */
  LCD_INIT();
  LCD_XY(0, 0);
  LCD_PUTS("Distance Meter!");
    //printf("Distance Meter with HC-SR04!\n");
  ...
  while (1)
  {
	  ...
	  if(sw == ON){
		  ...
		  if(sw == ON){
			  ...
			  uint16_t e3 = dist / 1000;
			  uint16_t e2 = dist % 1000 / 100;
			  uint16_t e1 = dist % 1000 % 100 / 10;
			  uint16_t e0 = dist % 1000 % 100 % 10;
...
NUCLEO 보드의 온보드 스위치(blue push button)를 누를 때 측정된 거리가 I2C CLCD에 표시되는 것을 확인한다.

![](./img/disttance_meter.png)
