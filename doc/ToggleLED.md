...
​	**- GPIO(PC13) : **입력으로 설정(온보드 SW 입력 처리)
...
GPIO 설정을 위해 다시 **Pinout & Configuration**탭으로 돌아가서 **Pin out view**에서 **PA5**를 GPIO_Output으로, PA13을 GPIO_Input으로 설정한다.

![](./img/stm32cubemx07.png)



**GPIO Mode & Configuration**의 **Configuration**의 **PC13**의 **GPIO mode**는 **Input mode**로, **GPIO Pull-up/Pull-down**은 Pull-up으로 설정한다.

![](./img/stm32cubemx08.png)

여기까지 설정을 반영한 코드를 생성하기위해 **Generate Code**를 클릭 하기 전 **STM32CubeMX**에서 생성한 프로젝트들을 저장해 둘 폴더를 만들어 두어야 한다. 그 위치는 편의상 **STM32CubeIDEworkspace_2.2.0** 폴더와 같은 폴더에, 폴더 이름은 **STM32CubeProjects**(폴더 이름은 다른 임의의이름을 사용해도 되나 그 위치와 이름을 정확히 기억하자 )로 만들어 둔다. 아래 그림에서도 **STM32CubeIDEworkspace_2.2.0**폴더가 있는 위치에 **STM32CubeProjects** 폴더가 만들어져 있는 것을 확인할 수 있다.
...
<img src="./img/stm32cubemx13.png" style="zoom:57%;" /> <img src="./img/stm32cubemx14.png" style="zoom:57%;" />
...
<img src="./img/import_prj_from_file_system.png" style="zoom:67%;" />

**STM32CubeIDE**의 **Project Explorer**창에 좀 전에 **import**한 ToggleLED프로젝트가 나타난 것을 확인할 수 있다.

![](./img/stm32cubeide_project_explorer.png)

이제 해당 프로젝트를 선택 후 Project메뉴의 Build Project를 선택하여 빌드 테스트를 진행한다.

![](D:\Dropbox\myGit\STM32Cube\doc\img\stm32cubeide_build_project_toggle_led.png)



![](./img/stm32cubeide_build_project.png)



**STM32CubeIDE**의 Project Explorer창의 ToggleLED프로젝트를 확장시켜 Core-Src-main.c를 찾아 연다.

![](./img/stm32cubeide_project_explore_ToggleLED_main.png)
...
지금 구현하려는 기능은 **NUCLEO-F103RB**타겟보드의 파란색 스위치를 누를 때 마다 **NUCLEO-F103RB**타겟보드의 녹색 LED의 상태가 켜짐과 꺼짐을 반복하도록 하는 것이다. 따라서 이 때 필요한 HAL(Hardware Abstract Layer:하드웨어 추상화 계층) 라이브러리는 **GPIO**로부터 입력되는 값을 읽어내는 HAL_GPIO_ReadPin( )과 **GPIO**로 신호를 출력하는 HAL_GPIO_WritePin( ) 2가지이다.
...
![](./img/stm32cubeide_run_run_toggle_led.png)

타겟보드의 USB 컨넥터 옆 LED의 깜박거림이 멈추면 프로그램 업로드가 완료된 것이다, 이제 파란색 푸시버튼 스위치를 눌러 누를 때마다 검정색 리셋 스위치 아래의 녹색 LED의 상태가 토글되는 지 확인한다.
