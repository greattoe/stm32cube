...
### HC-SR04 초음파 센서를 이용한 후방 장애물 검지기

**STM32CubeMX**와 **STM32CubeIDE**를 이용하여  **NUCLEO-F103RB**보드에  HC-SR04초음파 센서와 Buzzer를 연결하여, 장애물과의 거리에 따라 Buzzer를 통해 경고음을 출력하는 장애물 검지기를 구현해 보자.
...
Buzzer 모듈과 NUCLEO-F103RB 타겟보드와의 결선은 다음과 같이 연결한다.

| Buzzer | NUCLEO-F103RB |
| :----: | ------------- |
|   -    | GND           |
|   S    | PB7           |
...
**4. TIM3** Buzzer가 연결된 PB7으로 PWM 파형 출력을 위한 설정
...
PC7에 연결한 Buzzer로 PWM파형을 출력하기 위해 TIM3의 Clock Source를 Internal Clock으로, Channel2를 PWM Generation CH2로, Prescaler를 64-1로, 그리고 PC7을 TIM3_CH2로 설정한다.

![](./img/stm32cubemx_config_tim3_n_pc7.png)



Connectivity의 I2C1의 Mode에서 I2C를 I2C로 설정하고, PB8을 I2C1의 SCL로, PB9를 I2C1의 SDA로 설정한다.

![](./img/stm32cubemx_config_i2c1.png)
...
사용자 정의 라이브러리들 중 `uart2_printf.h`, `delay_us.h`와 `liquidcrystal_i2c.h`를 HC-SR04프로젝트 폴더의 Core-Inc 폴더에, `uart2_printf.c`,  `delay_us.c`와 `liquidcrystal_i2c.c`를 HC-SR04프로젝트 폴더의 Core-Src 폴더에 복사 후 Project Explorer에서 HC-SR04프로젝트 선택 후 [F5]키를 눌러 Refresh시킨다.
...
void Buzzer_Obstacle(uint16_t dist)
{
    if (dist < 300)
    {
        Buzzer_Tone(1000, 50);
...
```c
/* USER CODE BEGIN 2 */
  printf("Obstacle Detector with HC-SR04!\n");
...
HC-SR04 초음파 센서 앞에 넓고 평평한 장애물을 멀리서부터 서서히 가까이 가져감에 따라 Buzzer에서 출력되는 경고음의 변화를 확인한다.
